...
| P2 | Prod target mode is `production` | High | Schedules silently paused; pipelines skip prod guardrails; deployment lock disabled. SLA + governance bypass with no visible error. | `targets[t].mode == "production"` for every target named `^prod` | Resolved JSON |
| P3 | Prod `run_as` is a service principal | High | Prod jobs run as a human identity. Offboarding breaks prod; user's full access is the blast radius for any compromise. | `targets.prod.run_as.service_principal_name` set; `user_name` absent | Resolved JSON |
| P4 | Prod paths not user-bound | High | Prod artifacts under `/Workspace/Users/<email>/...`. Offboarding deletes prod state; collisions across deployers. | `targets.prod.workspace.root_path` does not match `^/Workspace/Users/` | Resolved JSON |
| P5 | Prod git-branch enforcement set | High | Any branch can be deployed straight to prod. Code-review gate becomes voluntary. | `targets.prod.git.branch` set | Resolved JSON |
...
| P8 | No `CAN_MANAGE` to broad groups | Critical | Every account user can edit, run, or delete the resource. Maximum insider blast radius. | No `permissions` entry at top, target, or resource level has `group_name` (case-insensitive) in the configured broad-groups set with `level in {CAN_MANAGE, IS_OWNER}`. Default broad set: `{users, account users}` — extend per-org. | Resolved JSON |
...
| P15 | Prod resources have explicit ownership | Medium | Creator offboards, resources orphan. No one can modify or destroy. Effective DoS on operations. | For every `jobs[*]` and `pipelines[*]` in the prod-resolved JSON: `IS_OWNER` is set at the resource level (top-level cannot grant `IS_OWNER`); `CAN_MANAGE` is set at the resource, target, or top level. Run policy against `bundle validate -t prod -o json` to apply only to prod. | Resolved JSON |
